...
class SelfAttention(nn.Module):
    ...
    def __init__(self, stimuli, heads=8, mask=False, kqnorm=False):
        """
        :param stimuli: stimuli dimension
        :param heads: number of attention heads
        :param mask: whether to mask out the upper half of the dot product matrix
        :param kqnorm: whether to normalize the keys and queries
        """

        super().__init__()

        assert stimuli % heads == 0, f'Embedding dimension ({stimuli}) should be divisible by nr. of heads ({heads})'
        ...
        self.tokeys    = nn.Linear(stimuli, stimuli, bias=False)
        self.toqueries = nn.Linear(stimuli, stimuli, bias=False)
        self.tovalues  = nn.Linear(stimuli, stimuli, bias=False)

        self.unifyheads = nn.Linear(stimuli, stimuli)
    ...
    def forward(self, x):
        ...
        b, t, e = x.size()
        h = self.heads
        assert e == self.stimuli, f'Input embedding dim ({e}) should match layer embedding dim ({self.stimuli})'
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, stimuli, heads, mask, seq_length, ff_hidden_mult=4, dropout=0.0, attention_type='default', sa_kwargs={}):
        super().__init__()

        if attention_type == 'default':
            self.attention = SelfAttention(stimuli, heads=heads, mask=mask, **sa_kwargs)
        elif attention_type == 'alt':
            self.attention = SelfAttentionAlt(stimuli, heads=heads, mask=mask)
        elif attention_type == 'wide':
            self.attention = SelfAttentionWide(stimuli, heads=heads, mask=mask)
        elif attention_type == 'gpt2':
            self.attention = SelfAttentionGPT2(stimuli, heads=heads, mask=mask)
        elif attention_type == 'narrow':
            self.attention = SelfAttentionNarrow(stimuli, heads=heads, mask=mask)
        # elif attention_type == 'relative':
        #     assert pos_embedding is not None
        #     self.attention = SelfAttentionRelative(stimuli, heads=heads, mask=mask, pos_embedding=pos_embedding)
        else:
            raise Exception(f'Self-attention type {type} not recognized.')

        self.mask = mask

        self.norm1 = nn.LayerNorm(stimuli)
        self.norm2 = nn.LayerNorm(stimuli)

        self.ff = nn.Sequential(

            nn.Linear(stimuli, ff_hidden_mult * stimuli),
            nn.ReLU(),
            nn.Linear(ff_hidden_mult * stimuli, stimuli)
        )

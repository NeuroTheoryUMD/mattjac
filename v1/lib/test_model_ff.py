...
def create_two_simple_networks(verbose=False):
    ...
    netA.to(netB)
    netB.to(output_11)
    model = m.Model(output_11, verbose)
    return model
...
##### NETWORK TESTS ####
def test_two_ff_creation():
    model = create_two_simple_networks(verbose=False)
...
def test_three_ff_creation():
    model = create_three_simple_networks(verbose=False)

    # test_correct_model_created
    assert len(model.networks) == 3
    assert len(model.networks[0].layers) == 1
    assert len(model.networks[1].layers) == 2
    assert len(model.networks[2].layers) == 1

    input_stim = model.inputs[0]
    netA = model.networks[0]
    netB = model.networks[1]
    netC = model.networks[2]
    output_11 = model.output

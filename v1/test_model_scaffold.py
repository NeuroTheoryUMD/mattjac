...
def test_scaffold_network_creation():
    model = create_scaffold_single_network(verbose=False)

    # test_correct_model_created
    assert len(model.networks) == 2
    assert len(model.networks[0].layers) == 3
    assert len(model.networks[1].layers) == 1

    input_stim = model.inputs[0]
    net_scaffold = model.networks[0]
    net_readout = model.networks[1]

...
def conv_scaffold(num_filters, num_inh_percent, num_iter, reg_vals, kernel_widths, kernel_heights):
    conv_layer_template = m.ConvolutionalLayer(
        num_filters=num_filters,
        num_inh=int(num_filters*num_inh_percent),
        filter_dims=kernel_widths,
        window='hamming',
        NLtype=m.NL.relu,
    ...
    # populate the list of layers for the number of iterations
    conv_layers = []
    for i in range(num_iter):
        conv_layers.append(m.ConvolutionalLayer().like(conv_layer_template))
    ...
    core_net = m.Network(layers=conv_layers,
...
def iter_scaffold(num_filters, num_inh_percent, num_iter, reg_vals, kernel_widths, kernel_heights):
    ...
    core_net = m.Network(layers=[iter_layer],
                         network_type=m.NetworkType.scaffold,
                         name='core')
...
#     fwsI = 7
#     XTreg0 = 0.0001
#     clayer0 = ConvLayer.layer_dict(
#         input_dims = data.stim_dims, num_filters=num_subs0, bias=False,
#         norm_type=1, num_inh=num_subs0//2, filter_dims=fws0, padding='same',
#         NLtype='relu', initialize_center=True)
#     clayer0['output_norm'] = 'batch'
#     clayer0['window'] = 'hamming'
#     clayer0['reg_vals'] = {'d2xt': XTreg0, 'center': Creg, 'bcs':{'d2xt':1} }
#     Ilayer = IterLayer.layer_dict(
#         num_filters=num_subsI, bias=False, num_iter=3, output_config='full',
#         norm_type=1, num_inh=num_subsI//2, filter_width=fwsI,
#         NLtype='relu', initialize_center=True)
#     Ilayer['output_norm'] = 'batch'
#     Ilayer['window'] = 'hamming'
#     scaffold_net =  FFnetwork.ffnet_dict(
#         ffnet_type='scaffold',
#         xstim_n = 'stim', layer_list = [clayer0, Ilayer] )
# 
#     clayer0 = TconvLayer.layer_dict(
#         input_dims = data.stim_dims, num_filters=num_subs, bias=False,
#         norm_type=1, num_inh=num_subs0//2, filter_dims=[1, fws0, 1, num_lags-num_iter],
#         NLtype='relu', initialize_center=True)
#     clayer0['output_norm'] = 'batch'
#     clayer0['window'] = 'hamming'
#     clayer0['reg_vals'] = {'d2xt': XTreg0, 'center': Creg, 'bcs':{'d2xt':1} }
#     Ilayer = IterTlayer.layer_dict(
#         num_filters=num_subsI, bias=False, num_iter=num_iter, output_config='full',
#         norm_type=1, num_inh=num_subsI//2, filter_width=fwsI, num_lags=2,
#         NLtype='relu', initialize_center=True)
#     Ilayer['output_norm'] = 'batch'
...
experiment_name = 'iter_exp_03'
experiment_desc = 'Comparing act_reg and num_iter on iterative networks.'
...
models = [{'conv_scaffold': conv_scaffold}, {'iter_scaffold': iter_scaffold}]


# grid search through the desired parameters 
grid_search = it.product(num_filterses, num_inh_percents, kernel_widthses, kernel_heightses, reg_valses, copy_weightses, freeze_weightses, include_MUses, is_multiexps, batch_sizes, num_iters, models)
print('====================================')
...
def generate_trial(prev_trials):
    trial_idx = 0
    for num_filters, num_inh_percent, kernel_widths, kernel_heights, reg_vals, copy_weights, freeze_weights, include_MUs, is_multiexp, batch_size, num_iter, model in grid_search:
        print('==========================================')
        print(num_filters, num_inh_percent, kernel_widths, kernel_heights, reg_vals, copy_weights, freeze_weights, include_MUs, is_multiexp, batch_size, num_iter, list(model.keys())[0])
        ...
        # make the model
        model = modelfunc(num_filters, num_inh_percent, num_iter, reg_vals, kernel_widths, kernel_heights)
